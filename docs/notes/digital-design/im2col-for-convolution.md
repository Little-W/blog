...
这是实现量化卷积时最容易忽略的边界。若输入张量是量化整数，数学上的零对应的是 `input_zero_point`，而不一定是字面值 `0`。

- 若 GEMM 前已经把输入减去 zero point，越界填充可写数学零（即变换后的字面 `0`）。
- 若 im2col 保留原始量化字节，越界位置应填 `input_zero_point`。

两种约定都可以，但 im2col、权重偏移、累加与 requantization 必须使用同一约定。混用“原始输入”和“已减 zero point 的 padding”会在边缘像素产生系统性偏差。
...
- `1×1`、全覆盖核和不规则通道数；
- 有/无对齐填充的行步长；
- 非零 input zero point 的量化输入。

最可靠的 oracle 是朴素卷积：对相同输入、权重、bias 和量化参数逐元素比较最终输出，而不只比较展开后的中间矩阵。这样能同时发现索引、组偏移、padding 与后处理约定不一致的问题。
