...
export class LAppView {
  ...
  public onPointerTracked(pointX: number, pointY: number): void {
    ...
    const viewY = (centerY - pointY) / rangeY;
    this._subdelegate.getLive2DManager().onDrag(
      Math.max(-1, Math.min(1, viewX)),
      Math.max(-1, Math.min(1, viewY))
    );
  }
  ...
  public onTouchesMoved(pointX: number, pointY: number): void {
    const ratio = this.getRenderRatio();
    const posX = pointX * ratio.x;
    const posY = pointY * ratio.y;
  ...
  public onTouchesEnded(pointX: number, pointY: number): void {
    ...
    const rect = this._subdelegate.getCanvas().getBoundingClientRect();
    lapplive2dmanager.onTap(
      x,
      y,
      Math.max(0, Math.min(1, pointX / Math.max(1, rect.width))),
      Math.max(0, Math.min(1, pointY / Math.max(1, rect.height)))
    );
  ...
  private getRenderRatio(): { x: number; y: number } {
    const canvas = this._subdelegate.getCanvas();
    return {
      x: canvas.width / Math.max(1, canvas.clientWidth),
      y: canvas.height / Math.max(1, canvas.clientHeight)
    };
  }
