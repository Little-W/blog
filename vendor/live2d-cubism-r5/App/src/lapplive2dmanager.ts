...
export class LAppLive2DManager {
  ...
  private releaseAllModel(): void {
    for (const model of this._models) model?.release();
  ...
  public onDrag(x: number, y: number): void {
    ...
    if (model && this._mouseTrackingEnabled) {
  ...
  public setMouseTrackingEnabled(enabled: boolean): void {
    ...
    if (!this._mouseTrackingEnabled) this._models[0]?.setDragging(0, 0);
  }

  public setIdleMotionEnabled(enabled: boolean): void {
    this._idleMotionEnabled = !!enabled;
    this._models[0]?.setIdleMotionEnabled(this._idleMotionEnabled);
  }

  public isMotionActive(): boolean {
    return !!this._models[0]?.isMotionActive();
  }

  public getMotionStatus(): Record<string, unknown> {
    return this._models[0]?.getMotionStatus() || {
      active: false,
      kind: 'loading',
      nextShowcaseIn: null
    };
  ...
  public onTap(
    ...
    const part = model?.playTapMotion(normalizedX, normalizedY) || 0;
    if (part) {
      (window as any).click_part = part;
      if (typeof (window as any).click_message === 'function') {
        (window as any).click_message();
      }
      window.dispatchEvent(new CustomEvent('yusen:live2d-tap', {
        detail: { part, x: normalizedX, y: normalizedY }
      }));
    }
  }
  ...
  private changeScene(index: number): void {
    ...
    instance.setIdleMotionEnabled(this._idleMotionEnabled);
  ...
  private _mouseTrackingEnabled: boolean;
